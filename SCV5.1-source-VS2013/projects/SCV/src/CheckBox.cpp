...
CheckBox::CheckBox(const scv::Point &p1, bool state, const std::string &str) : StateButton(state), Label(p1, Point( p1.x + 13 + FontFreeType::getInstance()->getStringLength(str) + 3,p1.y + 13), str) {
...
void CheckBox::display(void) {
   ...
   scissor->pushScissor(Scissor::Info(currPosition.x, kernel->getHeight() - (getHeight() + currPosition.y), getWidth() + freeType->getStringLength(Label::getString()) + 3, getHeight()));
...
void CheckBox::setString(const std::string& str) {
   static FontFreeType *freeType = FontFreeType::getInstance();
   Label::setString(str);
   setWidth(freeType->getStringLength(str) + 16);
}
