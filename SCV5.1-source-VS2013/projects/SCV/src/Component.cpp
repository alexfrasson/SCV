...
void Component::processMouse(const scv::MouseEvent &evt) {
   ...
   if (isInside(evt.getPosition()) && kernel->requestMouseUse(this)) {
      if (evt.getState() == MouseEvent::WHELLUP || evt.getState() == MouseEvent::WHEELDOWN) {
         ...
         if (isFocused()) {
            if (_isDragging) {
					Point newAbsolutePosition = evtPosition + _clickDiff;
					newAbsolutePosition.toFiveMultiple();
...
//@afrasson
void Component::processChar(const unsigned int &character, const int &modifier)
{
	if (isFocused())
		onChar(character, modifier);
}
//@afrasson
...
void Component::setResizingCursor(void) {
   ...
   //@afrasson
   static Kernel *kernel = Kernel::getInstance();
   if (_resizing[LEFT] || _resizing[RIGHT]) {
	   cursor->setCursor(kernel->glfwWindow, GLFW_HRESIZE_CURSOR);

   } else if (_resizing[TOP] || _resizing[BOTTOM]) {
	   cursor->setCursor(kernel->glfwWindow, GLFW_VRESIZE_CURSOR);
   }
   //@afrasson
}
