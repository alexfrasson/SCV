...
void ProgressBar::display(void) {
   ...
   scheme->applyDefaultModulate();
      ...
      //selection
      _cTexture->display(currPosition.x + 1, currPosition.y + 1, 9, static_cast<int>((getValue() / getMaxValue()) * (getWidth() - 4)) + 1, getHeight() - 2);
   ...
   StaticLabel::display(currPosition.x + getWidth() / 2 - FontFreeType::getInstance()->getStringLength(Label::getString()) / 2 , currPosition.y + getHeight() / 2 - 6, Label::getString());
}
