...
RadioButton::RadioButton(const scv::Point &p1, bool state, const std::string &str) : StateButton(state), Label(p1, Point(p1.x + 13 + FontFreeType::getInstance()->getStringLength(str) + 3,p1.y + 13), str) {
...
void RadioButton::display(void) {
   ...
   scissor->pushScissor(Scissor::Info(currPosition.x, kernel->getHeight() - (getHeight() + currPosition.y), getWidth() + FontFreeType::getInstance()->getStringLength(Label::getString()) + 3, getHeight()));
...
void RadioButton::setString(const std::string& str) {
   Label::setString(str);
   setWidth(FontFreeType::getInstance()->getStringLength(str) + 15);
}
