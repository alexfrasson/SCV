...
Spinner::Spinner(const scv::Point &p, unsigned int width, double minValue, double maxValue, double startValue, double stepValue) :
   ...
   _textField->setMinimumSize(Point(scv::FontFreeType::getInstance()->getStringLength(toString(maxValue)) + 4, getHeight()));
...
void Spinner::processKey(const scv::KeyEvent &evt) {
   ...
   if (!_receivingCallbacks || evt.state == KeyEvent::RELEASE) return;

   if (evt.keycode == GLFW_KEY_UP && (isFocused() || kernel->getFocusedComponent() == _textField)) {
      if (evt.state == KeyEvent::PRESS) IncrementStep();
   }
   else if (evt.keycode == GLFW_KEY_DOWN && (isFocused() || kernel->getFocusedComponent() == _textField)) {
      if (evt.state == KeyEvent::PRESS) DecrementStep();
   }
   else if (evt.keycode == GLFW_KEY_ENTER && (isFocused() || kernel->getFocusedComponent() == _textField)) {
      _textField->setString(toString(getValue()));
   }
   if (isFocused() || kernel->getFocusedComponent() == _textField) {
