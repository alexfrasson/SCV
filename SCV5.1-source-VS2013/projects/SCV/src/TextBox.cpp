...
void TextBox::refreshCursor(void) {
    ...
    if(isFocused()){
        ...
        for (int i = 0 ; i < _lineIndex.size()-1 ; i++) {
		  ...
		  _desloc.x = freeType->getStringLength(_str.substr(_lineIndex[translateLine], _currChar - _lineIndex[translateLine] + 1));
...
void TextBox::refreshText(void) {
    ...
    _lineIndex.push_back(0);	 
	 int value = freeType->getStringLength(_str.substr(firstCharLine, (lastCharLine - firstCharLine) + 1));
	 
    for (int i = 0 ; i < _str.size() ; i++) {
        lastCharLine = i;
        if (value > getWidth()-s_borderWidth || _str[i]=='\n') {
            firstCharLine = i;
            lastCharLine = i;
            translateLine++;
...
void TextBox::upLine(void) {
    ...
    for (int i = _lineIndex[_desloc.y - 1] , desloc = 0 ; i <= _lineIndex[_desloc.y] ; i++ , desloc++) {
        size = abs(freeType->getStringLength(_str.substr(_lineIndex[_desloc.y - 1],desloc)) - _desloc.x);
        if (size < menor) {
            menor = size;
            menorDesloc = desloc;
        }
...
void TextBox::processMouse(const scv::MouseEvent &evt) {
    ...
    if ((evt.getState() == MouseEvent::CLICK || evt.getState() == MouseEvent::HOLD) && isFocused()) {
        ...
        for (int i = _lineIndex[index.y] , desloc = 0 ; i <= _lineIndex[index.y + 1] ; i++ , desloc++) {
			  ...
			  size = abs(freeType->getStringLength(_str.substr(_lineIndex[index.y], desloc)) - index.x);				
            if (size < menor) {
                menor = size;
                menorDesloc = desloc;
            }
