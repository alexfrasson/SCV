...
void MenuBar::processKey(const scv::KeyEvent &evt) {
   ...
   if (evt.state == KeyEvent::RELEASE || _currSelectedMenu == -1) return;
   ...
   if (evt.keycode == GLFW_KEY_LEFT && !cMenu->_recentlyChange) {
   ...
   else if (evt.keycode == GLFW_KEY_RIGHT && ((cMenu->_currSelectedMenu != -1 && !cMenu->_list[cMenu->_currSelectedMenu]->hasSubMenus() && !cMenu->_recentlyChange) ||
...
void MenuBar::addMenu(ContextMenu *menu) {   
   ...
   menuH->registerMenu(menu);
	...
	int position = (_index.back() + (s_borderWidth * 2) + FontFreeType::getInstance()->getStringLength(menu->getString()));
