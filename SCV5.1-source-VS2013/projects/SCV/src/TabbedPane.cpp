...
void TabbedPane::addChild(Component *object, const std::string &label, bool resize) {
   ...
   scv::Point p = object->getRelativePosition();
   object->setRelativePosition(Point(p.x, p.y + s_barHeight));
   //@afrasson
   _index.push_back(_index.back() + (s_tabSpacing * 2) + FontFreeType::getInstance()->getStringLength(label));
...
void TabbedPane::processKey(const scv::KeyEvent &evt) {
   ...
   if (isFocused() && evt.state == KeyEvent::PRESS) {
      Point currPosition = getAbsolutePosition();
...
void TabbedPane::configPanel(void) {
   ...
   //scv::Point p = getChild(getCurrTabIndex())->getRelativePosition();
   //getChild(getCurrTabIndex())->setRelativePosition(Point(p.x, p.y + s_barHeight));
   //@afrasson
}
