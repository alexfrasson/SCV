...
std::string CodeGenerator::addChildren(scv::Component *child, std::string dad, bool customClass) {
   ...
   if(type == scv::Component::PANEL) { // Panel
      ...
      y = child->getRelativePosition().y + child->getHeight();
      if(customClass == false)
			code += "   scv::ComboBox *comboBox" + scv::toString(count[child->getType()]) + " = new scv::ComboBox(scv::Point(" + scv::toString(child->getRelativePosition()) + "), " + scv::toString(x - child->getRelativePosition().x) + ");\n";
      else
			code += "   ComboBox" + scv::toString(count[child->getType()]) + " *comboBox" + scv::toString(count[child->getType()]) + " = new ComboBox" + scv::toString(count[child->getType()]) + "(scv::Point(" + scv::toString(child->getRelativePosition()) + "), " + scv::toString(x - child->getRelativePosition().x) + ");\n";
...
std::string CodeGenerator::createAllocationCode(scv::Component *comp, bool customClass) {
   ...
   if(type == scv::Component::PANEL) { // Panel
      ...
      y = comp->getRelativePosition().y + comp->getHeight();
      if(customClass == false)
			code += "   scv::ComboBox *comboBox" + scv::toString(count[comp->getType()]) + " = new scv::ComboBox(scv::Point(" + scv::toString(comp->getRelativePosition()) + "), " + scv::toString(x - comp->getRelativePosition().x) + ");\n";
      else
			code += "   ComboBox" + scv::toString(count[comp->getType()]) + " *comboBox" + scv::toString(count[comp->getType()]) + " = new ComboBox" + scv::toString(count[comp->getType()]) + "(scv::Point(" + scv::toString(comp->getRelativePosition()) + "), " + scv::toString(x - comp->getRelativePosition().x) + ");\n";
...
std::string CodeGenerator::createCustomClassCodeH(scv::Component *comp) {
    ...
    if(type == scv::Component::PANEL) { // Panel
      ...
      code += "public:\n";
      code += "   ComboBox" + scv::toString(count[comp->getType()]) + "(scv::Point p1, unsigned int width);\n";
      count[comp->getType()] += 1;
...
std::string CodeGenerator::CallbacksCode(int type){
   ...
   //@afrasson
   if(type == scv::Component::CANVAS) {
      code += "\n   virtual void render(void);\n";
      code += "   virtual void update(void);\n";
   }
   //@afrasson
...
std::string CodeGenerator::createCustomClassCodeCpp(scv::Component *comp) {
   std::string code, childCode, ComponentName;
    ...
    if(type == scv::Component::PANEL) { // Panel
      ...
      code += "\n//" + ComponentName;
      code += "\n" + ComponentName + "::" + ComponentName + "(scv::Point p1, unsigned int width) : scv::ComboBox(p1, width) {\n}";
      count[comp->getType()] += 1;
   } else if(type == scv::Component::CANVAS) {
   ...
   //@afrasson
   if(type == scv::Component::CANVAS) {
      code += "void " + ComponentName + "::render(void) {\n}\n";
      code += "void " + ComponentName + "::update(void) {\n}\n";
   }
   //@afrasson
   if(type == scv::Component::TEXTBOX || type == scv::Component::TEXTFIELD)
...
std::string CodeGenerator::generateCodeAppCpp(void)
{
   ...
   applicationDotCpp += "\n";
   applicationDotCpp += "   setWindowTitle(\"";
   applicationDotCpp += SCV_VERSION_STRING;
   applicationDotCpp += " - FreeForm Interface Designer\");\n";
   applicationDotCpp += "}\n";
   applicationDotCpp += "\n";
